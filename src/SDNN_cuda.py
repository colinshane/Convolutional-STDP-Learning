...
class SDNN:
    ...
    def __init__(self, network_params, weight_params, stdp_params, total_time, DoG_params=None,
                 spike_times_train=None, spike_times_test=None,
    ...
    def init_net_struc(self, network_params):
        ...
        for i in range(self.num_layers):
            ...
            if network_params[i]['Type'] == 'input':
                ...
                d_tmp['H_layer'] = int(1 + floor((self.network_struc[i - 1]['H_layer'] + 2 *
                                                  d_tmp['pad'][0] - d_tmp['filter_size']) / d_tmp['stride']))
                d_tmp['W_layer'] = int(1 + floor((self.network_struc[i - 1]['W_layer'] + 2 *
                                                  d_tmp['pad'][1] - d_tmp['filter_size']) / d_tmp['stride']))
                d_tmp['shape'] = (d_tmp['H_layer'], d_tmp['W_layer'], d_tmp['num_filters'])
    ...
    def train_SDNN(self):
        ...
        for i in range(self.max_iter):
            ...
            if self.DoG:
                ...
                # (Image_number, H, W, M, time) to (H, W, M, time)
                st = self.spike_times_learn[self.curr_img, :, :, :, :]
            ...
            if self.curr_img + 1 < self.num_img_train:
                self.curr_img += 1
            else:
                self.curr_img = 0
